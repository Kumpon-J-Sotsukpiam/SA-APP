export const GET_ERRORS = 'GET_ERRORS'

export const RESET = 'RESET'

// Token Even Type
export const SET_CURRENT_TOKEN = 'SET_CURRENT_TOKEN'
export const UNSET_CURRENT_TOKEN = 'UNSET_CURRENT_TOKEN'

// Semester Even Type
export const ADD_SEMESTER = 'ADD_SEMESTER'
export const DELETE_SEMESTER = 'DELETE_SEMESTER'
export const SET_SEMESTER = 'SET_SEMESTER'
export const GET_SEMESTER = 'GET_SEMESTER'

// Student Even Type
export const ADD_STUDENT = 'ADD_STUDENT'
export const DELETE_STUDENT = 'DELETE_STUDENT'
export const SET_STUDENT = 'SET_STUDENT'
export const GET_STUDENT = 'GET_STUDENT'

// Course Even Type
export const ADD_COURSE = 'ADD_COURSE'
export const DELETE_COURSE = 'DELETE_COURSE'
export const SET_COURSE = 'SET_COURSE'
export const GET_COURSE = 'GET_COURSE'

// Class Even Type
export const ADD_CLASS = 'ADD_CLASS'
export const DELETE_CLASS = 'DELETE_CLASS'
export const SET_CLASS= 'SET_CLASS'
export const GET_CLASS = 'GET_CLASS'
export const CLEAR_CLASS = 'CLEAR_CLASS'
export const PUSH_STUDENT_IN_CLASS = 'PUSH_STUDENT_IN_CLASS'
export const PULL_STUDENT_IN_CLASS = 'PULL_STUDENT_IN_CLASS'

// Check-Student Even Type
export const ADD_CHECKIN = 'ADD_CHECKIN'
export const DELETE_CHECKIN = 'DELETE_CHECKIN'
export const GET_CHECKIN = 'GET_CHECKIN'
export const PUSH_STUDENT_IN_CHECKIN = 'PUSH_STUDENT_IN_CHECKIN'
export const PULL_STUDENT_IN_CHECKIN = 'PULL_STUDENT_IN_CHECKIN'

// Today Even Type
export const ADD_TODAY = 'ADD_TODAY'
export const DELETE_TODAY = 'DELETE_TODAY'
export const SET_TODAY = 'SET_TODAY'
export const GET_TODAY = 'GET_TODAY'

export const GET_FIRST = 'GET_FIRST'

// model before predict face
export const PUSH_MODEL = 'LOAD_MODEL'
export const PULL_MODEL = 'DELETE_MODEL'
export const PREDICT_FACE = 'PREDICT_FACE'